...
#include <pthread.h>
typedef struct { pthread_t thread; void *result; } Async;
...
ZC_FUNC int main(void);
Async fetch_data();
int32_t _impl_fetch_data();
// Global Generic Drop Dispatch
#define _z_drop(x) _Generic((x), \
    default: (void)0)


#line 5 "/tmp/opencode/test_async_minimal.zc"

ZC_FUNC int main(void)
{
    {

#line 6 "/tmp/opencode/test_async_minimal.zc"
    int32_t result = ({ Async _a = 
#line 6 "/tmp/opencode/test_async_minimal.zc"
fetch_data(); void* _r; pthread_join(_a.thread, &_r); (int32_t)(uintptr_t)_r; });
    }
}

#line 1 "/tmp/opencode/test_async_minimal.zc"
struct fetch_data_Args {
};
void* _runner_fetch_data(void* _args)
{
    struct fetch_data_Args* args = (struct fetch_data_Args*)_args;
    int32_t res = _impl_fetch_data();
    free(args);
...
#line 2 "/tmp/opencode/test_async_minimal.zc"
    return 42;
    }
}
Async fetch_data()
{
    struct fetch_data_Args* args = malloc(sizeof(struct fetch_data_Args));
    pthread_t th;
    pthread_create(&th, NULL, _runner_fetch_data, args);
    return (Async){.thread=th, .result=NULL};
}
